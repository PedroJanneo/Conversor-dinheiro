...
var excharge_rate = document.getElementById('exchange-rate');
...
exchange.addEventListener("click", () => {
    // Troca os valores dos campos 'from_currency' e 'to_currency'
    var temp = from_currency.value;
    from_currency.value = to_currency.value;
    to_currency.value = temp;

    // Atualiza os elementos de saída 'output_from' e 'output_to'
    output_from.innerText = `${input_amount.value} ${to_currency.value}`;
    output_to.innerText = `${to_amount} ${from_currency.value}`;

    // Calcula a conversão com as novas moedas
    calculate();
});
